...
/datum/mind/proc/give_uplink(silent = FALSE, datum/antagonist/antag_datum)
	...
	if(implant)
		...
		if(!silent)
			to_chat(traitor_mob, span_boldnotice("Your Symphionia Uplink has been cunningly implanted in you, for a small TC fee. Simply trigger the uplink to access it."))
