...
/obj/item/paperwork/examine_more(mob/user)
	...
	if(ishuman(user))
		...
		if(istype(viewer.mind?.assigned_role, stamp_job)) //Examining the paperwork as the proper job gets you some bonus details
			...
			if(stamped)
				. += span_info("It looks like these documents have already been stamped. Now they can be returned to Conglomeration of Colonists.")
			else
...
/obj/item/paperwork/service/Initialize(mapload)
	...
	detailed_desc += span_info(" You begin scanning over the document. This is a standard Symphionia NT-435Z3 form used for requests to Conglomeration of Colonists.")
...
/obj/item/paperwork/photocopy/examine_more(mob/user)
	...
	if(stamped)
		if(voided)
			...
			. += span_notice("The stamp on the front appears to be smudged and faded. Conglomeration of Colonists will probably still accept these, right?")
	else
...
/obj/item/paperwork/ancient/Initialize(mapload)
	...
	detailed_desc = span_notice("It's impossible to really tell how old these are or what they're for, but Conglomeration of Colonists might appreciate them anyways.")
