...
<<<<<<< HEAD
		priority_announce("Spatial object has reached peak energy charge in [initial(A.name)], please stand-by.", "Conglomeration of Colonists Wildlife Observations", has_important_message = TRUE)
=======
...
>>>>>>> Nova/master
	...
	if(charge_state < CHARGE_FINALWARNING && time_charged >= (max_charge * 0.5))
		...
		var/area/A = get_area(src)
<<<<<<< HEAD
		priority_announce("A rift is causing an unnaturally large energy flux in [initial(A.name)]. Stop it at all costs!", "Conglomeration of Colonists Wildlife Observations", ANNOUNCER_SPANOMALIES)
=======
