...
/datum/job/head_of_personnel/get_captaincy_announcement(mob/living/captain)
	return "Due to staffing shortages, newly promoted Acting Premier [captain.real_name] on deck!"
...
<h2>The Basics</h2>
<p>Card Trim - This is the job assigned to the card. The card's trim decides what Basic accesses the card can hold. Basic accesses cost nothing! Grey ID cards cannot hold Head of Staff or Premier trims. Silver ID cards can hold Head of Staff trims but not Premier trims and are in a box in the Head of Personnel's office and orderable from cargo. Gold ID cards can hold all access. The only guaranteed Gold ID card is the Premier's Spare, held in a golden safe on the bridge with access codes given to the station's highest ranking officer. All other gold ID cards are carried exclusively by Premiers.</p>
...
<ol>
<li>Grab an appropriate ID card. Head of Staff jobs require a silver ID card. Premier requires a gold ID card.</li>
...
<li>Select a template from the drop-down. This will apply as many <b>basic</b> accesses as possible based on the trim of the ID card and may apply wildcard accesses for Head of Staff and Premier trims. For best results, match the template to the ID card's trim.</li>
